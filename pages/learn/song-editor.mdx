...
- An UltraStar song has a defined grid size to position notes. The property in the txt file is called `BPM`.
  - NOTE: The `BPM` in an UltraStar txt file is not 'beat per minutes' but 'bars per minute in four-four time'. This means you have to multiply the value from the txt file with 4 to get the 'beats per minute'.
- Lower BPM values means a rougher grid. Higher BPM values mean a more fine-grained grid.
